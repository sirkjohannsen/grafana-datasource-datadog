...
describe('DataDogQueryCtrl', function () {
  ...
  beforeEach(angularMocks.inject(function ($rootScope, $controller, $q) {
    ...
    ctx.target = {};
    ctx.datasource.metricFindQuery = _sinon["default"].stub().returns(ctx.$q.when([]));
    ctx.panelCtrl = {
      panel: {}
    };
    ctx.panelCtrl.refresh = _sinon["default"].spy();
    ctx.ctrl = $controller(_module.QueryCtrl, {
      $scope: ctx.scope
    }, {
  ...
  describe('when adding function', function () {
    beforeEach(function () {
      ...
      ctx.ctrl.datasource.metricFindQuery = _sinon["default"].stub().returns(ctx.$q.when([{
        expandable: false
      }]));
